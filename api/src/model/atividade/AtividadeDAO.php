...
Class AtividadeDAO {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new SuperDAO('atividade');
	}

	//cadastrar
	function cadastrar (atividade $obj) {
		$this->sql = sprintf("INSERT INTO atividade(idprojeto, idprojetousuario, nome, descricao, status)
		VALUES(%d, %d, '%s', '%s', '%s')",
	...
	//atualizar
	function atualizar (Atividade $obj) {
		$this->sql = sprintf("UPDATE atividade SET idprojeto = %d, idprojetousuario = %d, nome = '%s', descricao = '%s', status = '%s', dataedicao = '%s' WHERE id = %d ",
			mysqli_real_escape_string($this->con, $obj->getObjprojeto()->getId()),
			mysqli_real_escape_string($this->con, $obj->getObjprojetousuario()->getId()),
			mysqli_real_escape_string($this->con, $obj->getNome()),
			mysqli_real_escape_string($this->con, $obj->getDescricao()),
	...
	//buscarPorId
	function buscarPorId (Atividade $obj) {
		$this->sql = sprintf("SELECT * FROM atividade WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	function listar () {
		...
		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Atividade' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($start, $limit) {
		$this->sql = "SELECT * FROM atividade limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );

		$this->superdao->resetResponse();

		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Atividade' , 'ListarPaginado' ) );
		}else{
	...
	//deletar
	function deletar (Atividade $obj) {
		$this->superdao->resetResponse();
		...
		$this->sql = sprintf("DELETE FROM atividade WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM atividade";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
