...
Class ProjetousuarioDAO {
	...
	public function __construct($con) {
		$this->con = $con;
		$this->superdao = new SuperDAO('projetousuario');
	}
	...
	//buscarPorId
	function buscarPorId (Projetousuario $obj) {
		$this->sql = sprintf("SELECT * FROM projetousuario WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
	...
	//listar
	function listar () {
		$this->sql = "SELECT * FROM projetousuario";
		$result = mysqli_query($this->con, $this->sql);

		$this->superdao->resetResponse();

		if(!$result) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Projetousuario' , 'Listar' ) );
		}else{
			while($row = mysqli_fetch_object($result)) {
	...
	function listarPaginado($start, $limit) {
		$this->sql = "SELECT * FROM projetousuario limit " . $start . ", " . $limit;
		$result = mysqli_query ( $this->con, $this->sql );

		$this->superdao->resetResponse();

		if ( !$result ) {
			$this->superdao->setMsg( resolve( mysqli_errno( $this->con ), mysqli_error( $this->con ), 'Projetousuario' , 'ListarPaginado' ) );
		}else{
	...
	//deletar
	function deletar (Projetousuario $obj) {
		$this->superdao->resetResponse();
		...
		$this->sql = sprintf("DELETE FROM projetousuario WHERE id = %d",
			mysqli_real_escape_string($this->con, $obj->getId()));
		$result = mysqli_query($this->con, $this->sql);
	...
	//quantidade total
	function qtdTotal() {
		$this->sql = "SELECT count(*) as quantidade FROM projetousuario";
		$result = mysqli_query ( $this->con, $this->sql );
		if (! $result) {
